...
func TestLoggerSync(t *testing.T) {
	// Create a writer that will return an error during Sync
...
func TestDebugEmptyMessage(t *testing.T) {
	...
	// Test with an invalid message for comparison
...
func TestConvertLogLevel(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// We log something at every level
			l.Debug("test")
			l.Info("test")
			l.Warn("test")
			l.Error("test")
			...
			// Check that logs appear only for the corresponding level and above
